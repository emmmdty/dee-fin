...
这些结果是 Ch4 的先行可靠性证据，不等于 v4 下游门控闭环已经完成。代码：
...
（Huang et al. 2310.01798：intrinsic self-correction 掉点、收益多源于金标）。故门控信号来源**必须二选一**并在
论文写清：**(a) 在线可得的无标签代理**（自一致性/校准置信/约束满足度，部署可算）；或 **(b) 显式定位为「离线
诊断/构建期质检工具」**（用 held-out 金标算 MRR delta，交付前一次性修图，不声称在线自愈）。DeepRefine
（2605.10488）的 Gain-Beyond-Draft 无金标奖励即为绕开 oracle 的先例（见 §5），我们与之的区别在**事件因果图 +
reachability + conformal 预算**，不在「无金标优化下游」本身。**不澄清此点，Ch4 会被审稿人一击致命。**
...
- **既有受控实验**：真实构建版 ECG 被 Ch2 堵死（抽取器 causal 召回 0.4%），因此先完成**受控
  reachability 扫描**（真 SeDGPL 排名 + 受控损失，`cross_stage.py`）。v4 Phase E 必须在 Phase A/B 后补
  gold/predicted/repaired 三图闭环，受控扫描不能替代真实图结果。
