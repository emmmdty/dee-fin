"""Frozen cross-stage data contracts for the EKG pipeline.
...
# --------------------------------------------------------------------------- #
# Nodes: an event record produced upstream (e.g. the MAVEN detection/canonicalization pipeline).
# --------------------------------------------------------------------------- #
...
class EventGraph(BaseModel):
    ...
    cycle detection) live in `ekg.core.graph` so this contract stays stable.
    """
...
class TemporalQuad(BaseModel):
    ...
    Retained so archived TKG experiments and old artifacts remain readable. The
    current headline uses CGEP event instances from ``ekg.succession`` instead.
