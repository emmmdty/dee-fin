...
Faithful to `tools.getTemplate` / `util.getDistance`, because the risk-aware edge
budget below is only a meaningful comparison against the real baseline.
...
what `select_nearest_edges` replaces -- a distance-budgeted selection that
...
gold ECG the main table runs on has no such score, so distance is the only signal there.)
...
from ekg.succession.data.cgep import RELATION_SURFACE, CgepInstance, CgepNode
...
def select_nearest_edges(edges: Sequence[Edge], max_edges: int = EDGE_BUDGET) -> list[Edge]:
    """Risk-aware edge budget: keep the `max_edges` edges nearest the query.
    ...
    *whole* graph (not the truncated head SeDGPL would keep) and retains the
    ...
    returned unchanged -- so on the instances the budget never bites, this and
...
def linearize(
    ...
    *,
    ...
    `truncate_edges` by default, `select_nearest_edges` for the risk-aware
    budget. The distance
...
def _distance_selector() -> EdgeSelector:
    """Risk-aware truncation: keep the edges nearest the query."""
