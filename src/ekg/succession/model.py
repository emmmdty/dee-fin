...
license, MAVEN build unreleased), because every reliability module hangs off these and
...
overwrites the token in `inputs_embeds`. The structural stream adds a third gate
...
embeddings. The selective head (`succession.selective`) layers a conformal
...
if TORCH_AVAILABLE:
    ...
    class GatedFusion(nn.Module):
        ...
        not to `x` -- a gate is not a residual, and the structural ablations depend on
        knowing which.
        """
        ...
        def residual(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
            ...
            The third gate uses this over a zero-initialised structural stream, so
    ...
    class EeCE(nn.Module):
        ...
        With ``enable_structure`` the structural stream is added: a per-token ``reach_anchor``
