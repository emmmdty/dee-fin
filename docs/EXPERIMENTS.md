...
| **PHASE_A 目标 / 公开 SOTA** | **≥.25 / 37.4** | **≥.20 / 32.9** | / 60.7 |

**结论**（如实）：① **α 曲线倒 U，全局 α=0.5 最优**，causal F1 从 .167 提到 **.234（+40%）**；
② **subevent/temporal 达标，causal .234 稳定卡在 .25 下方**；③ **给 causal 更高 α 反而降 F1**
（.234→.219→.205）——causal 瓶颈不在权重强度，再加权只会加剧过度预测；④ 逆频权重会抵消负采样，
故 neg-ratio 在 α=1 时近乎无效。**方向判断**：causal（CAUSE/PRECONDITION + 方向）比时序/子事件更难，
